...
export class ScheduleMessageWwebjsConsume {
  ...
  private async recoverScheduleSendBeforeAttemptLease(
    ...
    if (inspection.status === 'error') {
      if (inspection.reason === 'identity_conflict') {
        console.error(
          '[ScheduleMessageWwebjs] Conflicting immutable recovery identity discarded before attempt lease',
          {
            schedule_id: data.schedule_id,
            contact_id: data.contact_id,
    ...
    if (inspection.status === 'not_found' || inspection.state === 'reserved') {
      ...
      if (takeover.status === 'error') {
        if (takeover.reason === 'identity_conflict') {
          console.error(
            '[ScheduleMessageWwebjs] Reserved takeover identity conflict discarded',
            {
              schedule_id: data.schedule_id,
              contact_id: data.contact_id,
              message_id: data.message.message_id,
      ...
      try {
        ...
        if (adoption === 'stale' || adoption === 'terminal') {
          await idempotencyService
            ...
            .catch(() => undefined);
          console.error(
            '[ScheduleMessageWwebjs] Reserved ledger takeover was not adopted by the operational identity',
            {
              schedule_id: data.schedule_id,
              contact_id: data.contact_id,
              message_id: data.message.message_id,
  ...
  private async processScheduleMessage(
    ...
    try {
      ...
      if (this.isScheduleMessageInFlightLeaseError(error)) {
        console.info(
          '[ScheduleMessageWwebjsConsume] Deferring message without an active distributed lease',
          {
            schedule_id: data.schedule_id,
            message_id: data.message.message_id,
          }
  ...
  private async processScheduleMessageWithLease(
    ...
    try {
      ...
      if (isScheduleMessageSendAmbiguousError(error)) {
        ...
        console.warn(
          '[ScheduleMessageWwebjsConsume] Schedule provider result is ambiguous; suppressing failed status and automatic retry',
          {
            schedule_id: data.schedule_id,
            contact_id: data.contact_id,
            message_id: data.message.message_id,
  ...
  private async recoverDuplicateAmbiguousSchedule(
    ...
    if (!recovery) {
      ...
      if (legacyRecovery === 'identity_conflict') {
        if (beforeAttemptLease) {
          console.error(
            '[ScheduleMessageWwebjs] Legacy ledger recovery identity changed before terminal CAS',
            {
              schedule_id: data.schedule_id,
              contact_id: data.contact_id,
              message_id: data.message.message_id,
  ...
  private async ensureScheduleOperationalStateFromLedger(
    ...
    try {
      ...
      if (result === 'stale') {
        console.error(
          '[ScheduleMessageWwebjs] Ledger outcome matched but operational identity was stale; provider remains terminal and will not be retried',
          {
            schedule_id: data.schedule_id,
            contact_id: data.contact_id,
            message_id: data.message.message_id,
  ...
  private async markActiveProviderInvoked(
    ...
    try {
      ...
      // helper performs the exact provider/client assertion synchronously.
      this.activeSendDispatchGuards?.get(messageId)?.();
    } catch (error) {
      const reverted =
  ...
  private async transitionSchedulePreProviderFailureBestEffort(
    ...
    try {
      ...
      console.warn(
        '[ScheduleMessageWwebjsConsume] Suppressing failed status because the durable operational outcome rejected the transition',
        {
          schedule_id: data.schedule_id,
          contact_id: data.contact_id,
          message_id: data.message.message_id,
  ...
  private async recoverSucceededUpdate(
    ...
    try {
      ...
      if (
        !persistedScope ||
        persistedScope.source_provider !== 'wwebjs' ||
        persistedScope.worker_id !== currentConnectionScope.worker_id ||
        currentConnectionScope.source_provider !== 'wwebjs' ||

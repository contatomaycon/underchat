...
export class MessageSendWwebjsDlqConsume {
  ...
  private logPipelineEvent(
    ...
    if (level === 'error') {
      logger.error(payload, 'Wwebjs message send DLQ pipeline event');
      return;
    }

    if (level === 'warn') {
      logger.warn(payload, 'Wwebjs message send DLQ pipeline event');
      return;
    }

    logger.info(payload, 'Wwebjs message send DLQ pipeline event');
  }

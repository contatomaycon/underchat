...
export class NotificationMessageSendConsume {
  ...
  private async processNotificationMessage(
    ...
    if (claim.status === 'error') {
      if (claim.reason === 'identity_conflict') {
        console.error(
          '[NotificationMessageSend] Conflicting immutable idempotency identity rejected',
          {
    ...
    const beforeProviderInvoke: IProviderInvocationBoundary =
      async (): Promise<void> => {
        ...
        try {
          ...
          // The helper performs the exact socket assertion synchronously.
          assertDispatchActive();
        } catch (error) {
          const reverted =
    ...
    try {
      ...
      // provider boundary is socket/SDK/infrastructure availability and must
  ...
  private normalizeNotificationSucceededRecovery(
    ...
    if (
      ...
      console.error(
        '[NotificationMessageSend] Invalid succeeded phone-JID recovery discarded'
      );
      return null;
    }
    ...
    if (!recovery) {
      console.error(
        '[NotificationMessageSend] Conflicting succeeded phone-JID recovery discarded'
      );
    }
    return recovery;
  }
  ...
  private async recoverDuplicateNotificationSend(
    ...
    if (!recovery) {
      ...
      if (legacyRecovery === 'identity_conflict') {
        console.error(
          '[NotificationMessageSend] Legacy ambiguous recovery identity conflict discarded',
          {
            notification_id: data.notification_id,
            destination: this.buildNotificationDestination(data),
  ...
  private async claimNotificationSendAttempt(
    ...
    if (!accountId) {
      try {
        accountId = baileysEnvironment.baileysAccountId.trim();
      } catch {
        return null;
      }
    }

export const pt = {
  welcome: 'Bem-vindo à',
  please_login: 'Por favor, faça login na sua conta e comece a aventura',
  email: 'Email',
  password: 'Senha',
  forgot_password: 'Esqueceu sua senha?',
  login: 'Entrar',
  email_required: 'Email é obrigatório',
  password_required: 'Senha é obrigatória',
  login_error: 'Ocorreu um erro ao fazer login. Por favor, tente novamente.',
  login_invalid:
    'Credenciais de login inválidas. Verifique seu e-mail/usuário e senha.',
  chat_permission_denied:
    'Seu usuário não possui permissão para acessar o chat.',
  attendance_guard_locked_title: 'Fora do horário de atendimento',
  attendance_guard_locked_description:
    'Seu acesso está bloqueado temporariamente por horário de atendimento.',
  attendance_guard_today_windows: 'Horários de hoje',
  attendance_guard_next_unlock: 'Próxima liberação',
  network_error:
    'Erro de conexão. Verifique a rede e a URL do backend (EXPO_PUBLIC_BACKEND_URL).',
  app_title: 'Underchat',
  email_placeholder: 'email@email.com',
  password_placeholder: '············',
  show_password: 'Mostrar senha',
  hide_password: 'Ocultar senha',
  search_service: 'Pesquisar atendimento...',
  select_search_placeholder: 'Buscar opção...',
  in_service: 'Em Atendimento',
  awaiting_service: 'Aguardando atendimento',
  new: 'Novo',
  all: 'Todos',
  select: 'Selecionar',
  sort: 'Ordenar',
  apply: 'Aplicar',
  all_attendances: 'Todos',
  my_attendances: 'Meus atendimentos',
  queue: 'Fila',
  in_chat: 'Em atendimento',
  closed: 'Encerrados',
  chatbot: 'Chatbot',
  contact: 'Contato',
  private: 'Privado',
  send: 'Enviar',
  type_message: 'Digite uma mensagem',
  good_morning: 'Bom dia',
  good_afternoon: 'Boa tarde',
  good_evening: 'Boa noite',
  quick_message_no_text: 'Sem texto',
  is_typing: 'está digitando...',
  is_recording_audio: 'está gravando áudio...',
  scroll_to_latest: 'Ir para o final',
  chat_list_error: 'Erro ao carregar conversas.',
  messages_error: 'Erro ao carregar mensagens.',
  send_error: 'Erro ao enviar mensagem.',
  advanced_filters: 'Filtro avançado',
  filter_by_tag: 'Filtrar por etiqueta',
  filter_by_sector: 'Filtrar por setor',
  filter_by_channel: 'Filtrar por canal',
  filter_by_attendant: 'Filtrar por atendente',
  filter_by_name: 'Filtrar por nome',
  filter_by_name_placeholder: 'Digite o nome',
  filter_by_phone: 'Filtrar por telefone',
  filter_by_phone_placeholder: 'Digite o telefone',
  filter_by_protocol: 'Filtrar por protocolo',
  filter_by_protocol_placeholder: 'Digite o protocolo',
  last_name: 'Sobrenome',
  nickname: 'Apelido',
  birthday: 'Aniversário',
  phone_ddi: 'DDI',
  filter_date_start: 'Data inicial',
  filter_date_end: 'Data final',
  select_tag_filter: 'Selecione uma etiqueta',
  select_sector_filter: 'Selecione um setor',
  select_channel_filter: 'Selecione um canal',
  select_attendant_filter: 'Selecione um atendente',
  select_phone_ddi: 'Selecione o DDI',
  select_channel: 'Selecione o canal',
  select_sector: 'Selecione o setor',
  select_option: 'Selecione uma opção',
  select_date: 'Selecione a data',
  sort_by: 'Ordenar por',
  sort_order: 'Ordem',
  sort_in_chat: 'Ordenar atendimentos',
  sort_queue: 'Ordenar fila',
  sort_my_chats: 'Ordenar meus atendimentos',
  sort_chatbot: 'Ordenar chatbot',
  select_sort_field: 'Selecione o campo',
  select_sort_order: 'Selecione a ordem',
  last_message: 'Última mensagem',
  account: 'Conta',
  channel: 'Canal',
  name: 'Nome',
  phone: 'Telefone',
  status: 'Status',
  date: 'Data',
  attendant: 'Atendente',
  sector: 'Setor',
  started_at: 'Iniciado em',
  closed_at: 'Fechado em',
  ascending: 'Crescente',
  descending: 'Decrescente',
  cancel: 'Cancelar',
  filter: 'Filtrar',
  clear_filter: 'Limpar',
  clear_filters: 'Limpar filtros',
  no_conversations_found: 'Nenhuma conversa encontrada.',
  search_contacts: 'Buscar contatos...',
  add_contact: 'Adicionar contato',
  edit_contact: 'Editar contato',
  validate_contact: 'Validar contato',
  validate_contact_confirmation:
    'Deseja realmente validar este contato? Esta ação irá verificar se o telefone é válido no WhatsApp.',
  contact_validation_success: 'Contato validado com sucesso!',
  contact_validation_failed: 'Falha ao validar contato.',
  contact_add_error: 'Erro ao adicionar contato.',
  contact_edit_error: 'Erro ao editar contato.',
  contact_phone_required:
    'O contato precisa ter um telefone para iniciar uma conversa.',
  contact_must_be_validated:
    'O contato precisa ser validado para iniciar uma conversa.',
  contact_not_found: 'Contato não encontrado.',
  select_channel_sector: 'Selecionar canal e setor',
  select_channel_required: 'Selecione um canal para abrir a conversa.',
  open_conversation: 'Abrir conversa',
  attendance_only_online_required:
    'Este canal exige atendente online para abrir conversa.',
  chat_creation_error: 'Não foi possível abrir a conversa com este contato.',
  approved: 'Aprovado',
  add: 'Adicionar',
  remove: 'Remover',
  official_whatsapp_interactive: 'Resposta interativa',
  official_whatsapp_button: 'Botão',
  official_whatsapp_order: 'Pedido',
  official_whatsapp_unsupported: 'Mensagem não suportada',
  official_whatsapp_unsupported_hint:
    'Abra a conversa no WhatsApp para visualizar este conteúdo.',
  official_whatsapp_echo: 'Enviado pelo WhatsApp Business',
  official_whatsapp_referral: 'Origem da conversa',
  official_templates_loading: 'Carregando modelos aprovados da Meta...',
  official_templates_loading_error:
    'Não foi possível carregar os modelos aprovados da Meta.',
  official_templates_empty:
    'Este canal oficial não possui modelos aprovados disponíveis na Meta.',
  official_template_model: 'Modelo de template',
  select_official_template: 'Selecione o modelo aprovado',
  official_template_available_variables: 'Variáveis disponíveis',
  chatbot_message_variables_legend: 'Variáveis',
  template_variable_index: 'Índice',
  template_variable_button_index: 'Índice do botão',
  template_variable_value: 'Valor da variável',
  official_template_required_for_opening:
    'Selecione um modelo aprovado e preencha as variáveis para abrir a conversa oficial.',
  official_window_status: 'Janela oficial do WhatsApp',
  official_window_open_title: 'Janela de atendimento aberta',
  official_window_open_description:
    'A conversa pode ser aberta agora sem enviar um novo template.',
  official_window_awaiting_title: 'Aguardando resposta do contato',
  official_window_awaiting_description:
    'Já existe uma conversa oficial iniciada. Aguarde o contato responder ou o prazo de 24 horas terminar.',
  official_window_opening_awaiting_description:
    'Já existe uma conversa iniciada. Uma nova abertura será liberada quando o contato responder ou após 24 horas; depois do prazo, escolha um novo template.',
  official_window_uncertain_title: 'Aguardando confirmação do WhatsApp',
  official_window_uncertain_description:
    'O template foi encaminhado e aguarda confirmação do WhatsApp. Para evitar mensagem duplicada, um novo envio ficará bloqueado até a confirmação ou o fim do prazo de segurança.',
  official_window_opening_uncertain_description:
    'O template foi encaminhado e aguarda confirmação do WhatsApp. O envio permanece bloqueado temporariamente para evitar duplicidade.',
  official_window_closed_title: 'Janela oficial encerrada',
  official_window_no_history_title: 'Sem janela de atendimento ativa',
  official_window_closed_description:
    'A janela de 24h do WhatsApp oficial está fechada. Envie um template aprovado para reengajar o contato.',
  official_window_opening_closed_description:
    'Selecione um template aprovado para iniciar o contato com segurança.',
  official_window_no_history_description:
    'Este contato ainda não iniciou uma janela de 24h neste canal. Um template aprovado é necessário.',
  official_window_started_at: 'Iniciada ou renovada em',
  official_window_closes_at: 'Encerra em',
  official_window_template_sent_at: 'Template enviado em',
  official_window_reply_wait_expires_at: 'Nova abertura disponível em',
  official_window_template_requested_at: 'Template encaminhado em',
  official_window_uncertain_expires_at: 'Novo envio disponível em',
  official_opening_waiting_action: 'Aguardando resposta',
  official_opening_uncertain_action: 'Aguardando confirmação',
  official_window_remaining: 'Tempo restante',
  official_window_error_title: 'Não foi possível consultar a janela',
  official_window_refresh_required:
    'A janela encerrou enquanto você preenchia o formulário. Selecione um template para continuar.',
  try_again: 'Tentar novamente',
  official_template_insert_variable: 'Inserir variável',
  official_template_variables_resolved_hint:
    'As variáveis serão resolvidas com os dados atuais no momento do envio.',
  official_window_send_template: 'Enviar template',
  official_template_conversation_modal_title: 'Enviar template oficial',
  official_template_conversation_send: 'Enviar template',
  official_template_send_and_open: 'Enviar template e abrir',
  official_template_conversation_success:
    'Template oficial enviado com sucesso.',
  official_template_conversation_queued:
    'Template encaminhado para processamento. A confirmação do WhatsApp aparecerá na mensagem.',
  official_template_conversation_error:
    'Não foi possível enviar o template oficial.',
  official_message_send_failed: 'Não foi possível enviar esta mensagem.',
  official_message_send_failed_provider:
    'O provedor rejeitou a mensagem (código {code}).',
  official_message_send_failed_template_parameters:
    'A Meta rejeitou o template: a quantidade de parâmetros não corresponde ao modelo aprovado (132000).',
  official_message_send_confirmation_pending:
    'A confirmação do provedor não retornou. O WhatsApp pode ter recebido a mensagem; aguarde antes de tentar novamente.',
  processing_title: 'Processando',
  contact_photo_delete_error:
    'Erro ao remover a foto do contato. Tente novamente.',
  contact_label_template_remove_error: 'Erro ao remover etiqueta do contato.',
  document_type: 'Tipo de documento',
  cpf: 'CPF',
  cnpj: 'CNPJ',
  cpf_invalid: 'CPF inválido.',
  cnpj_invalid: 'CNPJ inválido.',
  ignore: 'Ignorar',
  not_ignore: 'Não ignorar',
  ignore_automation: 'Ignorar automação',
  ignore_totally: 'Ignorar totalmente',
  notes: 'Observações',
  selected_items: 'itens selecionados',
  save: 'Salvar',
  done: 'Concluir',
  validate: 'Validar',
  warning_title: 'Atenção',
  error_title: 'Erro',
  success_title: 'Sucesso',
  total: 'Total',
  success: 'Sucesso',
  failed: 'Falhas',
  name_required: 'Nome é obrigatório.',
  phone_required: 'Telefone é obrigatório.',
  phone_ddi_required: 'DDI é obrigatório.',
  select_photo: 'Selecionar foto',
  remove_photo: 'Remover foto',
  change_photo: 'Alterar foto',
  profile_photo: 'Foto do perfil',
  profile_photo_upload_success: 'Foto do perfil atualizada com sucesso!',
  profile_photo_upload_error: 'Erro ao enviar a foto do perfil.',
  profile_photo_remove_success: 'Foto do perfil removida com sucesso!',
  profile_photo_remove_error: 'Erro ao remover a foto do perfil.',
  chat_config_update_error:
    'Não foi possível atualizar as configurações do chat.',
  chat_user_status_update_error: 'Não foi possível atualizar seu status.',
  chat_sort_update_error: 'Não foi possível salvar a ordenação do chat.',
  image_permission_denied: 'Permissão de galeria negada.',
  logout: 'Sair',
  contacts: 'Contatos',
  about: 'Sobre',
  status_chat: 'Status',
  online: 'Online',
  offline: 'Offline',
  away: 'Ausente',
  busy: 'Ocupado',
  do_not_disturb: 'Não perturbe',
  settings: 'Configurações',
  notification: 'Notificação',
  notification_permission_denied:
    'Permissão de notificação negada. Ative nas configurações do dispositivo.',
  notification_enable_error:
    'Não foi possível ativar as notificações neste dispositivo.',
  notification_disable_error: 'Não foi possível desativar as notificações.',
  biometric_login: 'Login por biometria',
  biometric_login_description:
    'Use Face ID, Touch ID ou fingerprint para desbloquear sua sessão salva.',
  biometric_login_enable_title: 'Ativar login por biometria?',
  biometric_login_enable_description:
    'Depois do próximo acesso, você poderá desbloquear o Underchat com a biometria do aparelho.',
  biometric_login_enable: 'Ativar',
  biometric_login_not_now: 'Agora não',
  biometric_login_enabled: 'Login por biometria ativado.',
  biometric_login_disabled: 'Login por biometria desativado.',
  biometric_unlock_title: 'Desbloquear Underchat',
  biometric_unlock_description:
    'Confirme sua biometria para acessar a sessão salva neste aparelho.',
  biometric_unlock: 'Desbloquear',
  biometric_use_password: 'Entrar com email e senha',
  channel_status: 'Status dos Canais',
  channel_online: 'Conectado',
  channel_awaiting_qr: 'Aguardando leitura do QR code',
  channel_offline: 'Offline',
  channel_error: 'Erro',
  channel_stopped: 'Parado',
  channel_creating: 'Criando',
  channel_mismatched: 'Divergente',
  channel_unknown: 'Desconhecido',
  channels_with_issues: 'canais com problemas',
  max_length_200: 'O tamanho máximo é de 200 caracteres',
  no_contacts_found: 'Nenhum contato encontrado.',
  today: 'Hoje',
  yesterday: 'Ontem',
  sunday: 'Domingo',
  monday: 'Segunda-feira',
  tuesday: 'Terça-feira',
  wednesday: 'Quarta-feira',
  thursday: 'Quinta-feira',
  friday: 'Sexta-feira',
  saturday: 'Sábado',
  view_once_message: 'Visualização única',
  forwarded: 'Encaminhada',
  audio: 'Áudio',
  download: 'Baixar',
  image_download_success: 'Imagem baixada com sucesso.',
  image_download_error: 'Não foi possível baixar a imagem.',
  video_download_success: 'Vídeo baixado com sucesso.',
  video_download_error: 'Não foi possível baixar o vídeo.',
  file_download_success: 'Arquivo baixado com sucesso.',
  file_download_error: 'Não foi possível baixar o arquivo.',
  download_cancelled: 'Download cancelado.',
  document: 'Documento',
  location: 'Localização',
  contacts_other: 'contatos',
  received_contacts: 'Contatos recebidos',
  select_received_contact: 'Selecione um contato para abrir o cadastro',
  location_map_unavailable: 'Mapa indisponível',
  loading_more_messages: 'Carregando mensagens...',
  opening_conversation_title: 'Abrindo conversa...',
  opening_conversation_messages: 'Carregando mensagens da conversa.',
  opening_internal_conversation_messages:
    'Carregando mensagens do chat interno.',
  open_camera: 'Abrir câmera',
  open_emoji_keyboard: 'Abrir teclado de emoji',
  open_attachments: 'Abrir anexos',
  camera_permission_denied: 'Permissão da câmera negada.',
  protected_content: 'Conteúdo protegido',
  action_unavailable_by_permission: 'Ação indisponível por permissão',
  simultaneous_attendance_limit_message:
    'Você pode atender até {limit} atendimento(s) simultâneo(s). Seu limite foi atingido.',
  microphone_permission_denied: 'Permissão de microfone negada.',
  audio_recording_error: 'Não foi possível iniciar a gravação de áudio.',
  slide_left_to_cancel: 'Deslize para cancelar',
  release_to_cancel: 'Solte para cancelar',
  slide_up_to_lock: 'Deslize para cima para travar',
  recording: 'Gravando',
  delete_recording: 'Excluir gravação',
  pause_recording: 'Pausar gravação',
  resume_recording: 'Retomar gravação',
  send_recording: 'Enviar gravação',
  take_photo: 'Foto',
  record_video: 'Vídeo',
  documents: 'Documentos',
  photos: 'Fotos',
  videos: 'Vídeos',
  pick_video_source_title: 'Selecionar vídeo',
  pick_video_source_message: 'Escolha a origem do vídeo.',
  video_source_gallery: 'Galeria',
  video_source_camera: 'Câmera',
  document_size_exceeded: 'O arquivo excede o tamanho máximo para documento.',
  invalid_image_format: 'Formato de imagem inválido.',
  image_size_exceeded: 'A imagem excede o tamanho máximo permitido.',
  invalid_video_format: 'Formato de vídeo inválido.',
  video_size_exceeded: 'O vídeo excede o tamanho máximo permitido.',
  media_picker_open_error: 'Não foi possível abrir o seletor de mídia.',
  video_editor_opening: 'Abrindo editor de vídeo...',
  video_trim_error: 'Não foi possível processar o corte do vídeo.',
  video_trim_busy: 'Aguarde finalizar o editor de vídeo atual.',
  video_retry_source_missing:
    'O arquivo local do vídeo não está mais disponível. Selecione o vídeo novamente.',
  invalid_audio_format: 'Formato de áudio inválido.',
  audio_size_exceeded: 'O áudio excede o tamanho máximo permitido.',
  preview_media: 'Pré-visualização',
  send_media: 'Enviar mídia',
  select_contacts: 'Selecionar contatos',
  selected_contacts: 'Selecionados: {count}/{max}',
  max_contacts_selected: 'Você pode selecionar até {count} contatos.',
  select_at_least_one_contact: 'Selecione ao menos um contato.',
  annotation: 'Anotação',
  annotation_placeholder: 'Digite sua anotação',
  annotation_max_characters: 'Máximo de {count} caracteres.',
  location_permission_denied: 'Permissão de localização negada.',
  send_location: 'Enviar localização',
  send_location_current: 'Enviar localização atual',
  location_current: 'Localização atual',
  location_manual: 'Escolha manual',
  location_error: 'Não foi possível obter a localização atual.',
  location_search_placeholder: 'Pesquise ou insira um endereço',
  location_live_share: 'Compartilhar localização em tempo real',
  location_live_not_available:
    'Compartilhamento em tempo real será disponibilizado em breve.',
  location_nearby_places: 'Locais próximos',
  location_searching: 'Pesquisando...',
  location_precision_meters: 'Precisão de {meters}m',
  location_search_no_results: 'Nenhum endereço encontrado para esta busca.',
  location_latitude: 'Latitude',
  location_longitude: 'Longitude',
  location_name_placeholder: 'Nome do local (opcional)',
  location_address_placeholder: 'Endereço (opcional)',
  location_invalid_coordinates:
    'Informe coordenadas válidas para latitude e longitude.',
  transfer: 'Transferir',
  transfer_to: 'Transferir para',
  transfer_to_placeholder: 'Selecione como transferir',
  transfer_type_user: 'Usuário',
  transfer_type_sector: 'Setor',
  transfer_type_chatbot: 'Chatbot',
  transfer_annotation: 'Anotação',
  transfer_annotation_placeholder: 'Digite uma anotação opcional',
  keep_in_chat: 'Manter-se na conversa',
  keep_in_chat_description:
    'Ao transferir, você continua como atendente secundário.',
  send_message_on_transfer: 'Enviar mensagem ao transferir atendimento',
  send_message_on_transfer_description:
    'Desative para transferir sem enviar a mensagem automática de transferência.',
  transfer_successfully: 'Atendimento transferido com sucesso!',
  chat_transfer_error: 'Erro ao transferir atendimento.',
  channel_required: 'Selecione um canal.',
  user_required: 'Selecione um usuário.',
  sector_required: 'Selecione um setor.',
  chatbot_required: 'Selecione um chatbot.',
  transfer_sector_user_optional: 'Usuário do setor (opcional)',
  transfer_select_channel: 'Selecione o canal de destino',
  transfer_select_user: 'Selecione o usuário de destino',
  transfer_select_sector: 'Selecione o setor de destino',
  transfer_select_sector_user: 'Selecione o usuário do setor',
  transfer_select_chatbot: 'Selecione o chatbot de destino',
  bulk_mode_exit: 'Sair da seleção',
  bulk_select_all_filtered: 'Selecionar todos os filtrados',
  bulk_selection_filtered_hint:
    'A ação será aplicada a todos os atendimentos que correspondem aos filtros atuais.',
  bulk_selected_count: '{count} selecionado(s)',
  bulk_target_count: '{count} atendimento(s)',
  bulk_no_selection: 'Selecione ao menos um atendimento.',
  bulk_transfer: 'Transferir',
  bulk_close: 'Encerrar',
  bulk_transfer_dialog_title: 'Transferir atendimentos',
  bulk_close_dialog_title: 'Encerrar atendimentos',
  bulk_close_confirmation: 'Deseja realmente encerrar {count} atendimento(s)?',
  bulk_summary_title: 'Resultado da ação',
  bulk_failures_label: 'Falhas encontradas',
  bulk_failures_more: '+{count} falha(s) não exibida(s).',
  chat_bulk_action_success:
    'Ação concluída: {success}/{total} atendimento(s) processado(s).',
  chat_bulk_action_partial:
    'Ação parcial: {success}/{total} processado(s), {failed} falhou(aram).',
  chat_bulk_action_all_failed:
    'Não foi possível processar os atendimentos selecionados.',
  chat_bulk_action_no_targets:
    'Nenhum atendimento foi encontrado para os filtros atuais.',
  label: 'Etiqueta',
  select_label: 'Selecione a etiqueta',
  chat_label_update_success: 'Etiqueta atualizada com sucesso.',
  chat_label_update_error: 'Erro ao atualizar etiqueta.',
  search_messages: 'Pesquisar mensagens',
  search_messages_placeholder: 'Pesquisar mensagens',
  search_minimum_characters: 'Digite pelo menos {count} caracteres.',
  no_results_found: 'Nenhum resultado encontrado.',
  no_more_results: 'Sem mais resultados.',
  search_messages_error: 'Erro ao pesquisar mensagens.',
  attendance_history: 'Histórico de atendimento',
  attendance_history_load_error: 'Erro ao carregar histórico de atendimento.',
  no_attendance_history: 'Nenhum histórico de atendimento encontrado.',
  attendance_time: 'Tempo de atendimento',
  protocol: 'Protocolo',
  protocols: 'Protocolos',
  view_all_protocols: 'Ver todos os protocolos',
  protocol_type_transfer: 'Transferência',
  protocol_type_ura: 'URA',
  protocol_type_attendance: 'Atendimento',
  chatbot_type_input: 'Entrada',
  chatbot_type_output: 'Saída',
  chatbot_type_schedule: 'Agendamento',
  chatbot_type_webhook: 'Webhook',
  chat_queue_message: 'Para iniciar o atendimento clique em atender',
  chat_closed_message: 'Para reabrir o atendimento clique em reabrir',
  attend: 'Atender',
  reopen: 'Reabrir',
  join_conversation: 'Entrar na conversa',
  join_conversation_success: 'Você entrou na conversa com sucesso.',
  join_conversation_error: 'Não foi possível entrar na conversa.',
  leave_conversation: 'Sair da conversa',
  leave_conversation_confirmation:
    'Deseja realmente sair desta conversa? Você deixará de ser atendente secundário.',
  leave_conversation_success: 'Você saiu da conversa com sucesso.',
  leave_conversation_error: 'Não foi possível sair da conversa.',
  must_join_conversation_to_reply:
    'Entre na conversa para responder este atendimento.',
  only_secondary_can_leave:
    'Somente atendentes secundários podem sair desta conversa.',
  only_primary_can_transfer:
    'Somente o atendente primário, master/administrador ou usuário com permissão pode transferir este atendimento.',
  cannot_transfer_to_current_primary:
    'Não é possível transferir para o atendente primário atual.',
  only_primary_can_close:
    'Somente o atendente primário, master/administrador ou usuário com permissão pode encerrar este atendimento.',
  attendants_info: 'Informações dos atendentes',
  attendants_info_error:
    'Não foi possível carregar as informações dos atendentes.',
  primary_attendant: 'Atendente primário',
  primary_attendant_not_available: 'Atendente primário não identificado.',
  secondary_attendants: 'Atendentes secundários',
  no_secondary_attendants: 'Nenhum atendente secundário neste atendimento.',
  entered_at_label: 'Entrou em',
  attend_service: 'Atender',
  close_service: 'Encerrar atendimento',
  close_service_confirmation: 'Deseja realmente encerrar este atendimento?',
  close_service_send_message_toggle_label:
    'Enviar mensagem ao finalizar atendimento',
  close_service_send_message_toggle_description:
    'Desative para encerrar sem enviar a mensagem automática de finalização.',
  close_service_success: 'Atendimento encerrado com sucesso.',
  closure_comment_required: 'Informe o motivo do encerramento.',
  closure_reason_label: 'Motivo do encerramento',
  close_service_inform_closure_toggle_label: 'Informar motivo do encerramento',
  close_service_inform_closure_toggle_description:
    'Ative para informar um motivo.',
  reply: 'Responder',
  retry: 'Tentar novamente',
  replying_to: 'Respondendo mensagem',
  cancel_reply: 'Cancelar resposta',
  copy: 'Copiar',
  read_more: 'Ler mais',
  read_less: 'Ler menos',
  react: 'Reagir',
  edit: 'Editar',
  edit_message: 'Editar mensagem',
  delete: 'Apagar',
  forward: 'Encaminhar',
  message_actions: 'Ações da mensagem',
  react_to_message: 'Reagir à mensagem',
  more_actions: 'Mais ações',
  fixed_chats: 'Fixadas',
  pin: 'Fixar',
  unpin: 'Desfixar',
  pin_conversation: 'Fixar conversa',
  unpin_conversation: 'Desafixar conversa',
  chat_pin_success: 'Conversa fixada com sucesso!',
  chat_unpin_success: 'Conversa desfixada com sucesso!',
  chat_pin_error: 'Erro ao fixar conversa.',
  chat_unpin_error: 'Erro ao desafixar conversa.',
  message_pinned_by_user: '{name} fixou uma mensagem',
  message_pinned_by_phone: '{phone} fixou uma mensagem',
  message_unpinned_by_user: '{name} desfixou uma mensagem',
  message_unpinned_by_phone: '{phone} desfixou uma mensagem',
  message_pinned_default: 'Uma mensagem foi fixada',
  message_unpinned_default: 'Uma mensagem foi desfixada',
  message_ephemeral_activated_by_user:
    '{name} ativou as mensagens temporárias.',
  message_ephemeral_activated_by_phone:
    '{phone} ativou as mensagens temporárias.',
  message_ephemeral_activated_default:
    'As mensagens temporárias foram ativadas.',
  message_ephemeral_activated_description:
    'Todas as novas mensagens desaparecerão desta conversa 24 horas após o envio, exceto se salvas na conversa.',
  message_ephemeral_deactivated_by_user:
    '{name} desativou as mensagens temporárias.',
  message_ephemeral_deactivated_by_phone:
    '{phone} desativou as mensagens temporárias.',
  message_ephemeral_deactivated_default:
    'As mensagens temporárias foram desativadas.',
  chat_react_error: 'Não foi possível reagir à mensagem.',
  chat_edit_error: 'Não foi possível editar a mensagem.',
  chat_edited: 'Editado',
  chat_sent_outside_platform_short: 'WhatsApp',
  chat_sent_outside_platform_label: 'Enviada fora da Underchat',
  chat_view_edits: 'Visualizar edições',
  chat_edit_history: 'Histórico de edições',
  chat_original_message: 'Mensagem original',
  chat_edited_version: 'Versão editada',
  chat_delete_error: 'Não foi possível apagar a mensagem.',
  chat_forward_error: 'Não foi possível encaminhar a mensagem.',
  chat_forward_success: 'Mensagem encaminhada com sucesso.',
  chat_forward_partial_success:
    'Encaminhamento parcial: {sent} enviado(s), {failed} falhou(aram).',
  forward_worker_required_for_contacts:
    'Não foi possível encaminhar para contatos sem usuário autenticado.',
  battery_optimization_title: 'Otimização de Bateria',
  battery_optimization_message:
    'Para receber mensagens e notificações em tempo real mesmo com o app em segundo plano, desative a restrição de bateria para o Underchat.',
  battery_optimization_action: 'Remover Restrição',
  battery_optimization_dismiss: 'Agora Não',
  chat_attended_successfully: 'Chat atendido com sucesso',
  chat_reopened_successfully: 'Chat reaberto com sucesso',
  pick_service_success: 'Atendimento assumido com sucesso.',
  chat_status_update_error: 'Erro ao atualizar status do atendimento.',
  history_readonly: 'Modo somente leitura',
  view_protocol: 'Ver protocolo',
  view_labels: 'Ver etiquetas',
  forward_to_output_chatbot: 'Encaminhar para saída do chatbot',
  chat_forward_to_output_chatbot_update_failed:
    'Erro ao atualizar encaminhamento para saída do chatbot.',
  chat_attendance_inactivity_disable_action:
    'Desativar inatividade deste atendimento',
  chat_attendance_inactivity_enable_action:
    'Ativar inatividade deste atendimento',
  chat_attendance_inactivity_update_success:
    'Inatividade deste atendimento desativada com sucesso!',
  chat_attendance_inactivity_update_enable_success:
    'Inatividade deste atendimento ativada com sucesso!',
  chat_attendance_inactivity_update_failed:
    'Erro ao atualizar inatividade deste atendimento. Por favor, tente novamente.',
  chat_operator_reply_pending_alert_short: 'Cliente aguardando resposta',
  chat_operator_reply_pending_alert_banner:
    'Cliente aguardando resposta há {minutes} minuto(s).',
  close: 'Fechar',
  chat_action_ai_reply: 'Responder com IA',
  chat_action_transcribe: 'Transcrever',
  chat_ai_reply_title: 'Responder com Agente de IA',
  chat_ai_reply_reference_message: 'Mensagem de referência',
  chat_ai_reply_response_type: 'Tipo de resposta',
  chat_ai_reply_type_text: 'Texto',
  chat_ai_reply_type_audio: 'Áudio',
  chat_ai_reply_instructions: 'Instruções adicionais (opcional)',
  chat_ai_reply_instructions_placeholder:
    'Ex: Responda de forma objetiva e amigável...',
  chat_ai_reply_generate: 'Gerar resposta',
  chat_ai_reply_generating: 'Gerando resposta com IA...',
  chat_ai_reply_preview: 'Prévia da resposta',
  chat_ai_reply_audio_attached: 'Áudio gerado',
  chat_ai_reply_regenerate: 'Regenerar',
  chat_ai_reply_send: 'Enviar',
  chat_ai_reply_error: 'Erro ao gerar resposta com IA.',
  chat_ai_reply_send_audio_error: 'Erro ao enviar áudio gerado pela IA.',
  chat_ai_reply_message: 'Mensagem',
  chat_transcribe_title: 'Transcrição de Áudio',
  chat_transcribe_processing: 'Transcrevendo áudio...',
  chat_transcribe_error: 'Erro ao transcrever o áudio.',
  chat_transcribe_retry: 'Tentar novamente',
  chat_transcribe_cached: 'Em cache',
  chat_transcribe_copy: 'Copiar',
  chat_transcribe_copied: 'Copiado!',
};

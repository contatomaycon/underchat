...
const query = computed(() => ({
  ...
  description: debouncedSearch.value,
  price: debouncedSearch.value,
}));
...
const handleDelete = async () => {
  ...
  if (result) {
    await expenditureStore.listExpenditure(query.value);
  }
...
watch(
  query,
  async (q) => {
    await expenditureStore.listExpenditure(q);
  },
...
<template>
  <div v-if="isAdministrator">
    <VCard :title="$t('expenditures')" no-padding>
      <VCardText>
          ...
          <div class="d-flex gap-1">
            ...
            <IconBtn v-if="$canPermission(permissionsDelete)">
              <VTooltip
                ...
                activator="parent"
              >
                <span>{{ $t('delete_expenditure') }}</span>
              </VTooltip>
              <VIcon
                icon="tabler-trash"

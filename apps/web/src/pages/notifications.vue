...
useSnackbarCleanup(notificationsStore);
...
const loadWorkers = async () => {
  const result = await notificationsStore.getWorkers();
  if (result) {
...
const saveNotification = async () => {
  ...
  try {
    ...
    const result = await notificationsStore.updateNotifications(updateData);
    if (result) {
      notifications.value = {
...
<template>
  <div>
    ...
    <VSnackbar
      v-model="notificationsStore.snackbar.status"
      transition="scroll-y-reverse-transition"
      location="top end"
      :color="notificationsStore.snackbar.color"
    >
      {{ notificationsStore.snackbar.message }}
    </VSnackbar>
  </div>
</template>

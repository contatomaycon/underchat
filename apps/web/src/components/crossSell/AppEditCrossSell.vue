...
const updateCrossSell = async () => {
  const validateForm = await refFormEditCrossSell?.value?.validate();
  ...
  const payload: UpdateCrossSellRequest = {
    plan_product_id: plan_product_id.value,
    quantity: quantity.value,
    price: priceRaw.value,
  };

  const result = await crossSellStore.updateCrossSell(
    crossSellId.value,
    payload
  );
  if (result) {
...
watch(isVisible, async (visible) => {
  if (visible && crossSellId.value) {
    const crossSell = crossSellStore.list.find(
      (cs) => cs.plan_cross_sell_id === crossSellId.value
    );
    if (crossSell) {
      plan_product_id.value = crossSell.plan_product_id;
      quantity.value = crossSell.quantity;
      priceRaw.value = crossSell.price;
      priceDisplay.value = formatCurrency(crossSell.price);
    }
...
<template>
  <VDialog v-model="isVisible" max-width="600">
    ...
    <VForm ref="refFormEditCrossSell" @submit.prevent>
      <VCard :title="$t('edit_cross_sell')">
        <VCardText>
          <VRow>
            <VCol cols="12">
        ...
        <VCardText class="d-flex justify-end flex-wrap gap-3">
          ...
          <VBtn @click="updateCrossSell"> {{ $t('save') }} </VBtn>
        </VCardText>
      </VCard>
    </VForm>
  </VDialog>

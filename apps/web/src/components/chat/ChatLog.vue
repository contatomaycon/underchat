...
const hasQuotedPoll = (m: ListMessageResult): boolean =>
  !!(
    m.content?.quoted?.type === EMessageType.poll &&
    m.content.quoted.poll
  );

const getTotalVotes = (poll: { options?: Array<{ vote_count?: number | null }> | null } | null | undefined): number => {
  if (!poll?.options) return 0;
  return poll.options.reduce((sum, opt) => sum + (opt.vote_count || 0), 0);
};

const getOptionPercentage = (voteCount: number, totalVotes: number): number => {
  if (totalVotes === 0) return 0;
  return Math.round((voteCount / totalVotes) * 100);
};

const isOptionSelected = (messageId: string, optionIndex: number): boolean => {
  // TODO: Implementar lógica de seleção quando a funcionalidade de votação for implementada
  return false;
};

const onPollOptionClick = (
  message: ListMessageResult,
  optionIndex: number
): void => {
  // TODO: Implementar funcionalidade de votação
  console.log('Vote on poll:', message.message_id, 'option:', optionIndex);
};

const showPollVotes = (message: ListMessageResult): void => {
  // TODO: Implementar modal para mostrar detalhes dos votos
  console.log('Show votes for poll:', message.message_id);
};
            ...
            <div class="message-block">
              ...
              >
                ...
                <div class="quoted-content">
                  ...
                  <div v-if="hasQuotedPoll(msgGrp)" class="quoted-poll">
                    <VIcon size="22" color="primary">tabler-chart-bar</VIcon>
                    <div class="quoted-poll-info">
                      <span class="quoted-poll-question">
                        {{
                          msgGrp.content?.quoted?.poll?.question ||
                          t('poll_label', 'Enquete')
                        }}
                      </span>
                    </div>
                  </div>
                  ...
                  <div
                    v-if="
                      ...
                      msgGrp.content?.quoted?.type !== EMessageType.poll &&
              ...
              <div
                v-if="
                  msgGrp.content?.type === EMessageType.poll &&
                  msgGrp.content?.poll
                "
                :class="[
                  'poll-bubble',
              ...
              >
                ...
                <div class="poll-instruction">
                  ...
                  >
                    {{
                      msgGrp.content.poll.allow_multiple
                        ? 'tabler-checks'
                        : 'tabler-check'
                    }}
                  </VIcon>
                  {{
                    msgGrp.content.poll.allow_multiple
                      ? t('select_one_or_more_options', 'Selecione uma ou mais opções')
                      : t('select_one_option', 'Selecione uma opção')
                  }}
                </div>
                <div class="poll-options">
                  <div
                    v-for="(option, index) in msgGrp.content.poll.options"
                    :key="index"
                    class="poll-option"
                    :class="{
                      'poll-option--selected': isOptionSelected(
                        msgGrp.message_id,
                        index
                      ),
                    }"
                    @click="
                      !msgGrp.deleted &&
                        onPollOptionClick(msgGrp, index)
                    "
                  >
                    <div class="poll-option-content">
                      <div class="poll-option-indicator">
                        <VIcon
                          v-if="
                            isOptionSelected(msgGrp.message_id, index)
                          "
                          size="16"
                          color="white"
                        >
                    ...
                    <div class="poll-option-votes">
                      <div
                        ...
                        :style="{
                          width: `${getOptionPercentage(
                            option.vote_count || 0,
                            getTotalVotes(msgGrp.content.poll)
                          )}%`,
                        }"
                      ></div>
                      <div class="poll-option-vote-count">
                        <VAvatar
                          v-if="option.vote_count && option.vote_count > 0"
                          size="16"
                          class="me-1"
                        >
                          <VIcon size="10">tabler-user</VIcon>
                        </VAvatar>
                ...
                <div class="poll-footer">
                  <VBtn
                    ...
                    size="small"
                    color="success"
                    class="poll-show-votes"
                    @click="showPollVotes(msgGrp)"
                  >
                    {{ t('show_votes', 'Mostrar votos') }}
                  </VBtn>
                </div>
              </div>
...
.poll-bubble {
  ...
  .poll-option {
    ...
    &:hover:not(.poll-option--selected) {
  ...
  .poll-option-indicator {
    ...
    .poll-bubble--right & {
      border-color: rgba(17, 27, 33, 0.4);
    }
  }

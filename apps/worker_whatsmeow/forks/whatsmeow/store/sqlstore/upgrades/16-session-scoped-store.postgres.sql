...
-- The v15 PostgreSQL schema already has a UUID session_id. Retain legacy
-- tables for rollback/read-only audit, but copy their durable contents into
-- the generic v16 projection before new runtime code can write it.
...
INSERT INTO whatsapp_session_revision (session_id, revision_id, provider, status, source, schema_version, codec_version, format)
SELECT d.session_id, 1, 'whatsmeow', 'active', 'legacy_postgres', 16, 1, 'whatsmeow-sqlstore-v15'
FROM whatsmeow_device AS d
ON CONFLICT (session_id, revision_id) DO NOTHING;
INSERT INTO whatsapp_device (session_id, revision_id, jid, lid, facebook_uuid, registration_id, noise_key, identity_key, signed_pre_key, signed_pre_key_id, signed_pre_key_sig, adv_key, adv_details, adv_account_sig, adv_account_sig_key, adv_device_sig, platform, business_name, push_name, lid_migration_ts, next_pre_key_id)
SELECT d.session_id, 1, d.jid, d.lid, d.facebook_uuid, d.registration_id, d.noise_key, d.identity_key, d.signed_pre_key, d.signed_pre_key_id, d.signed_pre_key_sig, d.adv_key, d.adv_details, d.adv_account_sig, d.adv_account_sig_key, d.adv_device_sig, d.platform, d.business_name, d.push_name, d.lid_migration_ts,
       LEAST(16777216, GREATEST(1, COALESCE((SELECT MAX(p.key_id) + 1 FROM whatsmeow_pre_keys AS p WHERE p.jid = d.jid), 1)))
FROM whatsmeow_device AS d
ON CONFLICT (session_id, revision_id) DO NOTHING;
INSERT INTO whatsapp_identity_keys (session_id, revision_id, their_id, identity)
SELECT d.session_id, 1, s.their_id, s.identity FROM whatsmeow_identity_keys AS s JOIN whatsmeow_device AS d ON d.jid = s.our_jid ON CONFLICT DO NOTHING;
INSERT INTO whatsapp_pre_keys (session_id, revision_id, key_id, key, uploaded)
SELECT d.session_id, 1, s.key_id, s.key, s.uploaded FROM whatsmeow_pre_keys AS s JOIN whatsmeow_device AS d ON d.jid = s.jid ON CONFLICT DO NOTHING;
INSERT INTO whatsapp_signal_sessions (session_id, revision_id, their_id, session)
SELECT d.session_id, 1, s.their_id, s.session FROM whatsmeow_sessions AS s JOIN whatsmeow_device AS d ON d.jid = s.our_jid ON CONFLICT DO NOTHING;
INSERT INTO whatsapp_sender_keys (session_id, revision_id, chat_id, sender_id, sender_key)
SELECT d.session_id, 1, s.chat_id, s.sender_id, s.sender_key FROM whatsmeow_sender_keys AS s JOIN whatsmeow_device AS d ON d.jid = s.our_jid ON CONFLICT DO NOTHING;
INSERT INTO whatsapp_app_state_sync_keys (session_id, revision_id, key_id, key_data, timestamp, fingerprint)
SELECT d.session_id, 1, s.key_id, s.key_data, s.timestamp, s.fingerprint FROM whatsmeow_app_state_sync_keys AS s JOIN whatsmeow_device AS d ON d.jid = s.jid ON CONFLICT DO NOTHING;
INSERT INTO whatsapp_app_state_version (session_id, revision_id, name, version, hash)
SELECT d.session_id, 1, s.name, s.version, s.hash FROM whatsmeow_app_state_version AS s JOIN whatsmeow_device AS d ON d.jid = s.jid ON CONFLICT DO NOTHING;
INSERT INTO whatsapp_app_state_mutation_macs (session_id, revision_id, name, version, index_mac, value_mac)
SELECT d.session_id, 1, s.name, s.version, s.index_mac, s.value_mac FROM whatsmeow_app_state_mutation_macs AS s JOIN whatsmeow_device AS d ON d.jid = s.jid ON CONFLICT DO NOTHING;
...
SELECT d.session_id, 1, s.their_jid, s.first_name, s.full_name, s.push_name, s.business_name, s.redacted_phone FROM whatsmeow_contacts AS s JOIN whatsmeow_device AS d ON d.jid = s.our_jid ON CONFLICT DO NOTHING;
INSERT INTO whatsapp_chat_settings (session_id, revision_id, chat_jid, muted_until, pinned, archived)
SELECT d.session_id, 1, s.chat_jid, s.muted_until, s.pinned, s.archived FROM whatsmeow_chat_settings AS s JOIN whatsmeow_device AS d ON d.jid = s.our_jid ON CONFLICT DO NOTHING;
INSERT INTO whatsapp_message_secrets (session_id, revision_id, chat_jid, sender_jid, message_id, key)
SELECT d.session_id, 1, s.chat_jid, s.sender_jid, s.message_id, s.key FROM whatsmeow_message_secrets AS s JOIN whatsmeow_device AS d ON d.jid = s.our_jid ON CONFLICT DO NOTHING;
INSERT INTO whatsapp_privacy_tokens (session_id, revision_id, their_jid, token, timestamp, sender_timestamp)
SELECT d.session_id, 1, s.their_jid, s.token, s.timestamp, s.sender_timestamp FROM whatsmeow_privacy_tokens AS s JOIN whatsmeow_device AS d ON d.jid = s.our_jid ON CONFLICT DO NOTHING;
INSERT INTO whatsapp_nct_salt (session_id, revision_id, salt)
SELECT d.session_id, 1, s.salt FROM whatsmeow_nct_salt AS s JOIN whatsmeow_device AS d ON d.jid = s.our_jid ON CONFLICT DO NOTHING;
INSERT INTO whatsapp_lid_map (session_id, revision_id, lid, pn)
SELECT d.session_id, 1, s.lid, s.pn FROM whatsmeow_lid_map AS s CROSS JOIN whatsmeow_device AS d ON CONFLICT DO NOTHING;
INSERT INTO whatsapp_event_buffer (session_id, revision_id, ciphertext_hash, plaintext, server_timestamp, insert_timestamp)
SELECT d.session_id, 1, s.ciphertext_hash, s.plaintext, s.server_timestamp, s.insert_timestamp FROM whatsmeow_event_buffer AS s JOIN whatsmeow_device AS d ON d.jid = s.our_jid ON CONFLICT DO NOTHING;
INSERT INTO whatsapp_retry_buffer (session_id, revision_id, chat_jid, message_id, format, plaintext, timestamp)
SELECT d.session_id, 1, s.chat_jid, s.message_id, s.format, s.plaintext, s.timestamp FROM whatsmeow_retry_buffer AS s JOIN whatsmeow_device AS d ON d.jid = s.our_jid ON CONFLICT DO NOTHING;

...
var (
	...
	// ErrReadPartitionsAutoTopicCreationUnsupported is returned when a broker
	// cannot negotiate Metadata v6 for a topic-scoped lookup. Older metadata
	// requests do not carry the allow_auto_topic_creation bit, so proceeding
	// would make fail-closed behavior impossible to prove.
	ErrReadPartitionsAutoTopicCreationUnsupported = errors.New(
		"topic-scoped ReadPartitions requires Metadata v6 with auto topic creation disabled",
	)
)
...
// cluster.
//
// UnderChat intentionally keeps this false. ConsumerGroup, DialLeader and
// partition readers all use ReadPartitions internally. Allowing a metadata
// lookup to create a missing worker topic would silently replace a deleted
// durable queue with an empty one and make the provider look connected while
// outbound delivery is broken.
const ReadPartitionsAutoTopicCreationEnabled = false

func readPartitionsMetadataRequest(topics []string) topicMetadataRequestV6 {
	return topicMetadataRequestV6{
		Topics:                 topics,
		AllowAutoTopicCreation: ReadPartitionsAutoTopicCreationEnabled,
	}
}

func validateReadPartitionsMetadataVersion(version apiVersion, topics []string) error {
	if version != v6 && len(topics) > 0 {
		return ErrReadPartitionsAutoTopicCreationUnsupported
	}
	return nil
}
...
func (c *Conn) ReadPartitions(topics ...string) (partitions []Partition, err error) {
	...
	if err := validateReadPartitionsMetadataVersion(metadataVersion, topics); err != nil {
		return nil, err
	}
	...
	err = c.readOperation(
		func(deadline time.Time, id int32) error {
			...
			case v6:
				return c.writeRequest(metadata, v6, id, readPartitionsMetadataRequest(topics))
			default:

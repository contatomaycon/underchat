...
func TestReadPartitionsMetadataDisablesAutoTopicCreation(t *testing.T) {
	topics := []string{"worker.worker-1.send.message"}
	request := readPartitionsMetadataRequest(topics)

	if ReadPartitionsAutoTopicCreationEnabled {
		t.Fatal("ReadPartitions auto topic creation must remain disabled")
	}
	if request.AllowAutoTopicCreation {
		t.Fatal("metadata request unexpectedly enables auto topic creation")
	}
	if len(request.Topics) != 1 || request.Topics[0] != topics[0] {
		t.Fatalf("metadata request topics = %#v, want %#v", request.Topics, topics)
	}
	if ErrReadPartitionsAutoTopicCreationUnsupported == nil {
		t.Fatal("legacy metadata fail-closed error is unavailable")
	}
	if err := validateReadPartitionsMetadataVersion(v6, topics); err != nil {
		t.Fatalf("Metadata v6 was rejected: %v", err)
	}
	if err := validateReadPartitionsMetadataVersion(v1, topics); !errors.Is(
		err,
		ErrReadPartitionsAutoTopicCreationUnsupported,
	) {
		t.Fatalf("topic-scoped Metadata v1 error = %v", err)
	}
	if err := validateReadPartitionsMetadataVersion(v1, nil); err != nil {
		t.Fatalf("cluster-wide Metadata v1 listing was rejected: %v", err)
	}
}

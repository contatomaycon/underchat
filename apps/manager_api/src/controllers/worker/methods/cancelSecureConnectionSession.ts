...
export const cancelSecureConnectionSession = async (
  ...
  try {
    logSecureConnectionHttpFlow(
      'manager.http.secure_connection.cancel.received',
      {
    ...
    const response = await useCase.cancelAuthenticated(t, {
      accountId: tokenJwtData.account_id,
      workerId: request.params.worker_id,
      token: request.params.token,
    });

    logSecureConnectionHttpFlow('manager.http.secure_connection.cancel.done', {
      request_id: request.id,
    ...
    return sendResponse(reply, {
      message: t('worker_secure_connection_session_cancelled'),
      httpStatusCode: EHTTPStatusCode.ok,
      data: response,
    });
  } catch (error) {
    logSecureConnectionHttpFlow('manager.http.secure_connection.cancel.error', {
      request_id: request.id,
      worker_id: request.params.worker_id,
      account_id: tokenJwtData.account_id,
      token_hash: tokenHash,

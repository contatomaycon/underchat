// ==================== Painel de configuração de extração de imagem ====================
    ...
    methods: {
        ...
        async loadPresets() {
            ...
            try {
                ...
                console.error('Falha ao carregar a predefinição:', e);
            } finally {
        ...
        async applyPreset(presetDomain) {
            ...
            try {
                ...
                alert('Falha ao aplicar predefinição: ' + e.message);
            }
        },
        ...
        <div class="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg shadow-sm" @click="closeMenu">
            ...
            <div class="px-4 py-3 border-b dark:border-gray-700 flex justify-between items-center cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
                ...
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="font-semibold text-gray-900 dark:text-white">🖼️ Extração de imagem</h3>
                    <span v-if="isEnabled" class="px-2 py-0.5 text-xs bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300 rounded font-medium">Habilitado</span>
                    <span v-else class="px-2 py-0.5 text-xs bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 rounded">Não ativado</span>
                    ...
                    <div class="relative">
                        ...
                        <div v-if="showPresetMenu" class="absolute right-0 mt-1 w-80 max-h-96 overflow-y-auto bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg shadow-lg z-10">
                            <div v-if="currentPreset && currentPreset.available" class="px-3 py-2 bg-blue-50 dark:bg-blue-900/30 border-b dark:border-gray-700 text-xs text-blue-700 dark:text-blue-300">
                                <div class="font-medium">Atualmente usando o padrão</div>
                                <div class="mt-0.5">{{ currentPreset.name }}</div>
                            </div>
                            <div v-if="loadingPresets" class="px-3 py-6 text-center text-sm text-gray-400">carregando...</div>
                            <div v-else-if="availablePresets.length > 0" class="divide-y dark:divide-gray-700">
                                <template v-if="availablePresets.filter(p => !p.is_special).length > 0">
                                    <div class="px-3 py-1.5 bg-gray-50 dark:bg-gray-900/50 text-xs font-medium text-gray-500">padrões do site</div>
                                ...
                                <template v-if="availablePresets.filter(p => p.is_special).length > 0">
                                    <div class="px-3 py-1.5 bg-gray-50 dark:bg-gray-900/50 text-xs font-medium text-gray-500">Predefinição universal</div>
                                    <button v-for="preset in availablePresets.filter(p => p.is_special)" :key="preset.domain"
                                            @click="applyPreset(preset.domain)"
                                            class="w-full text-left px-3 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                            ...
                            <div v-else class="px-3 py-6 text-center text-sm text-gray-400">Nenhuma predefinição disponível ainda</div>
                        </div>
                    </div>
            ...
            <div v-show="!collapsed" class="p-4 space-y-4">
                ...
                <div v-if="!isEnabled" class="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-lg p-3 text-center">
                    <div class="text-gray-500 dark:text-gray-400 text-sm">A extração de imagem está desativada. Depois de habilitar,AI As imagens da resposta serão extraídas e devolvidas automaticamente.</div>
                    <button @click="toggleEnabled" class="mt-2 px-4 py-1.5 text-sm font-medium text-white bg-blue-500 hover:bg-blue-600 rounded-lg transition-colors">Ativar extração de imagem</button>
                </div>
                ...
                <template v-else>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Seletor de imagens</label>
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">CSS Seletor, o padrão é img</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Seletor de contêiner <span class="text-gray-400 font-normal">(Opcional)</span></label>
                            <input type="text" :value="imageConfig.container_selector || ''" @input="updateField('container_selector', $event.target.value || null)" placeholder="Deixe em branco para usar o contêiner de resposta"
                                   class="w-full border dark:border-gray-600 px-3 py-2 rounded-md text-sm font-mono bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-400 focus:border-transparent">
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Limitar o intervalo de pesquisa de imagens</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Modo de extração</label>
                            <select :value="imageConfig.mode" @change="updateField('mode', $event.target.value)"
                                ...
                                <option value="first">Somente o primeiro</option>
                                <option value="last">Só o último</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">tamanho máximo (MB)</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Atraso anti-vibração (Segundo)</label>
                    ...
                    <div class="border-t dark:border-gray-700 pt-4">
                        <div class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Opções avançadas</div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                                <div>
                                    <div class="text-sm font-medium text-gray-700 dark:text-gray-300">Aguarde a imagem carregar</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">Aguarde a imagem carregar antes de extraí-la</div>
                                </div>
                                <label class="toggle-label scale-90">
                            ...
                            <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                                <div>
                                    <div class="text-sm font-medium text-gray-700 dark:text-gray-300">Converter Blob foto</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">Vai blob: URL converter para data URI</div>
                                </div>
                                <label class="toggle-label scale-90">
                            ...
                            <div class="md:col-span-2 flex items-center gap-4 p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                                <div class="flex-1">
                                    <div class="text-sm font-medium text-gray-700 dark:text-gray-300">Tempo limite de carregamento</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">Tempo máximo de espera para carregar uma única imagem</div>
                                </div>
                                <div class="flex items-center gap-2">
                    ...
                    <div class="border-t dark:border-gray-700 pt-4 flex justify-end">
                        <button @click="$emit('test-image-extraction')"
                            ...
                            Extração de imagem de teste                         </button>
                    </div>
                </template>
            </div>
        </div>

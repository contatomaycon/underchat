-- Complete the safe legacy-volume restoration in the same transaction that
-- invalidates a failed PostgreSQL candidate. The original function preserved
-- the session bytes but could leave the control-plane worker in `recreating`
-- with an unacknowledged, already-online legacy runtime.
...
BEGIN
  ...
  WHERE migration.migration_id = p_migration_id
    ...
    AND worker.lifecycle_operation_id IS NOT DISTINCT FROM migration.lifecycle_operation_id
  ...
  WHERE worker.worker_id = p_worker_id
    AND worker.session_storage = 'legacy_volume'
    AND worker.lifecycle_operation_id IS NOT DISTINCT FROM v_migration.lifecycle_operation_id
  ...
  WHERE runtime.worker_id = p_worker_id
    ...
    AND runtime.runtime_generation IN (
      v_migration.source_runtime_generation,
      v_migration.target_runtime_generation
    )
...
COMMENT ON FUNCTION public.invalidate_legacy_volume_migration_revision(
  ...
  'Atomically invalidates a failed PostgreSQL candidate and restores the exact healthy legacy runtime to terminal online state.';

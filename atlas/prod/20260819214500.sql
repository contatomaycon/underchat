-- Allow a browser-authenticated Baileys session to replace only the
-- original, never-linked pairing draft. Baileys persists locally generated
-- bootstrap keys before a companion is linked, so a pristine draft is not
-- physically empty. The exception remains closed when any JID/LID, ADV
-- account proof or canonical companion fingerprint exists.
--
-- The cumulative v17 primitive is repeated so all existing promotion,
-- lifecycle, reservation and identity checks remain unchanged.

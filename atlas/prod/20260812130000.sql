-- Preserve provider-specific bootstrap failures through the generic,
-- transactionally fenced WhatsApp handoff rollback. The value is still an
-- application-owned token: messages, stack traces and provider payloads are
-- never accepted into durable error_code columns.
...
BEGIN
  v_error_code := CASE
    WHEN p_error_code ~* '^(handoff|whatsapp|wwebjs|baileys|whatsmeow)_[a-z0-9_.:-]{1,90}$'
      THEN p_error_code

-- A cold recreate binds the successor container into worker_runtime while the
-- channel deliberately remains in `recreating`. Permit that exact, fully
-- fenced runtime to activate before the lifecycle is finalized. Destructive
-- and stopped states remain rejected, and every container/generation/writer

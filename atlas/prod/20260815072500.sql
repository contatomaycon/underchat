-- The legacy migration wrapper used a data-modifying function inside a CTE
-- and then updated the same revision tuple from another CTE in the same SQL
-- command. PostgreSQL correctly rejects that ambiguous double update with
-- SQLSTATE 27000. Keep both operations in the same transaction, but execute
-- them as two ordered PL/pgSQL statements and require the exact revision to be
-- marked once.
...
COMMENT ON FUNCTION public.open_whatsapp_session_revision(
  ...
  'Opens ordinary or fenced legacy-volume revisions, marking the exact migrated revision in a separate ordered statement to avoid ambiguous tuple updates.';

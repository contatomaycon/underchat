-- QR pairing may resume the exact empty staging revision created for any
-- supported WhatsApp provider. The previous resumable boundary admitted only
-- Baileys, which left valid WWebJS and WhatsMeow drafts permanently rejected
-- as non-empty. Generalize that narrow exception without weakening the strict
-- fallback or mutating provider-native session data.
...
BEGIN
  ...
  -- Volume-backed runtimes, unsupported providers, and canonical empty
  -- PostgreSQL sessions continue through the strict implementation unchanged.
    ...
    AND (
      ...
      OR (
        ...
        AND NOT EXISTS (
          ...
          WHERE device.session_id = p_worker_id
        )
      )
    )
    AND NOT EXISTS (
